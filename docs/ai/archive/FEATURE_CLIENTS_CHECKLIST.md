# Фича `clients` — чек-лист состояния (архивная копия)

Файл заархивирован: фича закрывает критерии MVP и больше не находится в активной разработке.

Оригинал: `docs/ai/FEATURE_CLIENTS_CHECKLIST.md`
...
- Персистентность после перезагрузки страницы (пока)
...
- Archive status:
  ...
  - Примечание: ручные проверки UI/UX в браузере пока не удалось инструментально подтвердить, но MVP покрыт unit-тестами.

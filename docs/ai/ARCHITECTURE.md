...
**Связка документов (контракт):** структурные артикулы — **`EVE_ARCH_INDEX.md`**; оглавление семейств Eve-BL / Eve-UX / Eve-PERF / Eve-SEC / бэкенд — **`EVE_INDEXES.md`**. Любое изменение паттерна в коде → согласованное обновление соответствующего индекса и этого файла (якоря **#fixed-layout**, **#slot-body**, **#dialog-life**, **#a11y-ui-kit** и т.д.).
...
- Временные sandbox/demo-страницы (например `demo-feature`) допускаются как отдельные lazy-route для обкатки паттернов; обязательно помечать как временные и удалять после переноса решений в рабочие фичи.
...
### Верификация критичных паттернов (как проверить / регресс)

**`app-dialog` (`libs/ui-kit` → `@ui-kit/dialog`):**
- **Как проверить:** на `/clients`, `/organizations`, `/catalog/*`, `/catalog/products` открыть content-модалку (форма/детали) — тело с `slot="body"` видно; confirm — текст + кнопки из `confirmText`/`cancelText`; Tab циклится внутри панели (`role="dialog"` + `aria-modal="true"` + `uiFocusTrap`); Esc закрывает при `closeOnEsc` по умолчанию.
- **Регресс:** пустое тело при открытой модалке → **`DOCS_SYNC_RULES.md`** (диагностика «пустое тело dialog»); в шаблоне диалога не дублировать проекцию тела в ветках `@if (mode === 'confirm')` / `@else` (**SLOT-003**, **DIALOG-LIFE-009**).

**Stable layout (CRUD, FIXED-LAYOUT-001):**
- **Как проверить:** список + открытие модалки без заметного CLS; таблица в `.table-wrap`, скролл у `.dialog-body`, фиксированные min-height из токенов.
- **Регресс:** появление локальной разметки оверлея (`modal-backdrop` / `modal-panel` и аналоги) во `features/**/*.html` вместо `app-dialog` без оформленного временного решения (**см. DOCS_SYNC_RULES**) — отклонение от целевого паттерна. (Legacy-классы `.modal-*` удалены из рабочего SCSS; остаются только как текст аварийного сценария в `DOCS_SYNC_RULES`.)
...
Базовый стандарт: переиспользуемый `app-dialog` (`mode="content"`/`mode="confirm"`). Локальная разметка `modal-backdrop/modal-panel` допустима только как временный аварийный fallback по протоколу из `DOCS_SYNC_RULES.md`, с обязательным возвратом к единому компоненту (текущий статус: возврат выполнен после фикса проекции в `app-dialog`).
...
2. **Фиксированная геометрия модалки**  
   - Задаётся **`app-dialog`** (`size="sm|md|lg|xl"`) + токены `--modal-min-height` / `--modal-max-height` в **`libs/ui-kit` `dialog.component.scss`**, чтобы окно **не меняло высоту при смене вкладки**.  
   - Ширина/высота — через модификаторы `.dialog-panel--*`; **`overflow: hidden`** у панели, скролл — у `.dialog-body` и внутри вкладки формы.
...
4. **Flex-цепочка (кратко)**  
   `.dialog-panel` (column) → `.dialog-body` → проецируемая обёртка → хост формы / карточка → `card-body` → `form` (`flex: 1`, `min-height: 0`) → ряд вкладок (`flex-shrink: 0`) → **панель вкладки** (`flex: 1`, `min-height: 0`, скролл) → **действия** (`flex-shrink: 0`).
...
- Форма клиента (вкладки + та же схема модалки): `apps/web/src/app/features/clients/client-form.component.{ts,html,scss}` и `clients-page.component.scss` (`app-dialog` + `::ng-deep .dialog-body app-client-form`)  
...
Единая шкала задаётся **`app-dialog`** (`size` prop) и стилями **`libs/ui-kit/src/lib/dialog/dialog.component.scss`** (`.dialog-panel--sm|md|lg|xl`):

- `sm` — короткие быстрые формы (quick-add, 1–3 поля)
- `md` — обычные CRUD-формы справочников
- `lg` — расширенные формы (несколько секций, но без перегруза)
- `xl` — большие формы с вкладками/многими разделами

Назначение по текущему проекту:
- `categories`, `materials`, `part-types` → `size="md"`
- `clients` → `size="lg"`
- `organizations`, `products` (редактор товара) → `size="xl"`
- `@ui-kit/quick-add-dialog` по умолчанию `size="sm"` (повышать до `md/lg` только при реальной необходимости)

Запрещено:
- задавать произвольные локальные размеры модалок в каждой фиче без согласования со шкалой `app-dialog` `sm/md/lg/xl`;
...
3. Модалки:
   - единый **`app-dialog`** (backdrop-layer + анимации в ui-kit)
   - единая геометрия через `size="sm/md/lg/xl"`
   - фиксированный визуальный ритм: контент скроллится в `.dialog-body`, футер действий закреплён снизу
4. Карточка "Детали":

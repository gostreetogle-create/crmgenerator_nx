...
| **ROUTES-004** | Централизованный routing map | Единая карта standalone + вложенный каталог; чанки фич — см. **PERF-LAZY-ROUTES-001**. Временные demo-маршруты допускаются только как отдельные lazy entry и с пометкой на удаление. | `app.routes.ts`; `catalog-shell.component.ts`; `feature-example-page.component.ts` | 2026-03-21 | [ARCHITECTURE — routes-map](./ARCHITECTURE.md#routes-map) |
...
| **THEME-012** | Переключение тем (Лев/Пантера/Орёл) | Глобальный селектор темы в header; `localStorage` + классы `dark`/`eagle` на `documentElement`; значения берутся из `design-tokens`. | `apps/web/src/app/app.ts`; `libs/design-tokens/src/styles/tokens.scss`; `apps/web/src/app/features/ui-catalog/ui-catalog.component.html` | 2026-03-21 | [ARCHITECTURE — theme-dark](./ARCHITECTURE.md#theme-dark) |

...
| **BL-PLAYGROUND-001** | Временная эталонная фича (demo) | Учебный стенд под типичный поток проекта: smart-page + dumb-form, таблица, детали, confirm-удаление; используется для безопасной обкатки паттернов перед переносом в боевые фичи. | `features/feature-example/feature-example-page.component.{ts,html,scss}`; `feature-example-form.component.{ts,html,scss}` | [ARCHITECTURE — routes-map](./ARCHITECTURE.md#routes-map) |
